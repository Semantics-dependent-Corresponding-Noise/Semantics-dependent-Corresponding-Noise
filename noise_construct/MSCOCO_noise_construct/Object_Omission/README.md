...
| :--- | :--- |
| `coco_object_omission_threaded_batch_doubao_noise_construct.py` | Training noise generation (100% error). |
| `coco_object_omission_threaded_batch_doubao_test_noise_construct.py` | Test noise generation. |
| `merge_files.py` | Merges split files. |
| `coco_object_omission_noise.py` | Generates partial error noise. |
| `coco_object_omission_img5txt_noise.py` | Generates all error noise. |
...
Run the command `python coco_object_omission_threaded_batch_doubao_noise_construct.py` to generate base noise files containing **100% erroneous noise descriptions**, processing images sequentially in **batches of 1000**. The output files, named `train_caps_5_per_image_part{file_number}.txt`, will be saved to the `./train_file/` directory.
...
**Generate Test Noise**
Run the command `python coco_object_omission_threaded_batch_doubao_test_noise_construct.py` to directly generate a complete test set noise file.

### Phase III: Generate Noise with Specified Ratio

**1. Partial Error (Mixed)**
Run the command `python coco_object_omission_noise.py` to generate dataset variations where **not all descriptions** for a given image are erroneous (standard setting: 1 image, 5 captions, with partial errors).

**2. All Error (Total)**
Run the command `python coco_object_omission_img5txt_noise.py` to generate dataset variations where **all descriptions** for a given image are erroneous (setting: 1 image, 5 captions, 100% errors).
